...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        #show='headings' скрываем нулевую колонку таблицы
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email'),
                                 height=45, show='headings')
        #Указываем размеры и расположение столбцов 
        ...
        btn_search = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.search_img,
                               command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)
        
        #Кнопка обновления
        self.refresh_img = tk.PhotoImage(file='./img/refresh.png')
        btn_refresh = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.refresh_img,
                               command=self.view_records)
        btn_refresh.pack(side=tk.LEFT)

        #Создаем функцию прокручивания
